...
class App extends Component {
  ...
  getCategoryBasedImages = () => {
    const {activeTabId} = this.state
    const filteredImagesList = imagesList.filter(
      eachImageItem => eachImageItem.category === activeTabId,
    )
  ...
  renderHeader = () => {
    const {score, timeElapsedInSec} = this.state
    return (
      <ul className="header-container">
        ...
        <li className="score-and-timer-container">
          ...
          <img
            ...
            alt="timer"
          />
          <p className="highlight">{timeElapsedInSec} sec</p>
        </li>
      </ul>
    )
  }
  ...
  onSelectImage = selectedImageData => {
    const {randomImagesData} = this.state
    if (randomImagesData.category === selectedImageData.category) {
      this.setState(prevState => ({
    ...
    this.setState({
      randomImagesData: imagesList[Math.floor(Math.random() * 30 + 1)],
    })
  }

  renderGameView = () => {
    const {activeTabId, randomImagesData} = this.state
    const filteredImages = this.getCategoryBasedImages()

    return (
      <div className="game-content-container">
        <div className="image-container">
          <img
            src={randomImagesData.imageUrl}
            alt="match"
        ...
        <ul className="images-list">
          {filteredImages.map(eachItem => (
            <ImageItem
              ...
              onChangeImage={this.onSelectImage}
            />
          ))}
        </ul>
      </div>
  ...
  playAgainClicked = () => {
    this.setState({score: 0, timeElapsedInSec: 60})
  }

  render() {
    const {timeElapsedInSec, score} = this.state
    return (
